...
def move_diagonally(pirate):
    ...
    X = int(X)
    hp = 0.44 #high probability
    lp = 0.06 #low probability
    if int(dp[0]) < X/2 :
        if int(dp[1]) == 0:
    ...
    for i in range(1,4):
        ...
        if s != "__empty__":
            ...
            if int(pirate.getID())%5 == 1:
                if (pirate.trackPlayers()[2+i] == "OppCapturing"):
                    return circleAround(x,y,1,pirate)
                else:
            ...
            if "oppCaptured" in pirate.trackPlayers():
                for i in range(1,4):
                    try:
                        if pirate.trackPlayers()[2+i] == "oppCaptured":
                            x = int(TeamSig[5+i].split(',')[0])
                            y = int(TeamSig[5+i].split(',')[1])
                            return moveTo(x,y,pirate)
                        
                        elif pirate.trackPlayers()[2+i] == "oppCapturing" and pirate.trackPlayers()[i-1] == "myCaptured":
                            x = int(TeamSig[5+i].split(',')[0])
                            y = int(TeamSig[5+i].split(',')[1])
                            return moveTo(x,y,pirate)
                    except:
